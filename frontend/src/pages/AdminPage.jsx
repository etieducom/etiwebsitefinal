...
const AdminPage = () => {
  ...
  const handleNavSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        order: parseInt(navForm.order) || 0
      };
      
      if (editingNav) {
        await axios.put(`${API}/navigation/${editingNav.id}`, payload);
  ...
  const handleEditNav = (item) => {
    ...
    setNavForm({
      ...
      is_dropdown: item.is_dropdown || false,
      icon: item.icon || ""
    });
  ...
  const handleDeleteNav = async (id) => {
    if (!window.confirm("Delete this navigation item? This will also delete its sub-items.")) return;
    try {
  ...
  const handleSeedDefaultNav = async () => {
    try {
      const res = await axios.post(`${API}/navigation/seed-default`);
      toast.success(res.data.message);
      fetchData();
    } catch (error) {
      toast.error("Failed to seed default navigation");
    }
  };
